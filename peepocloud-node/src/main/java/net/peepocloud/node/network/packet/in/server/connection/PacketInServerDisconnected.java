...
public class PacketInServerDisconnected implements PacketHandler<SerializationPacket> {
    @Override
    public int getId() {
        return 28;
    }
    ...
    public void handlePacket(NetworkPacketSender networkParticipant, SerializationPacket packet, Consumer<Packet> queryResponse) {
        ...
        participant.getServers().remove(serverInfo.getComponentName(), serverInfo);
        participant.getStartingServers().remove(serverInfo.getComponentName());

        System.out.println(PeepoCloudNode.getInstance().getLanguagesManager().getMessage("network.disconnect.server.other")
