...
public class PacketInBungeeGroupDeleted implements PacketHandler<PacketOutMinecraftGroupDeleted> {
    @Override
    public int getId() {
        return 3;
    }
    ...
    public void handlePacket(NetworkPacketSender networkParticipant, PacketOutMinecraftGroupDeleted packet, Consumer<Packet> queryResponse) {
        PeepoCloudNode.getInstance().getBungeeGroups().remove(packet.getName());
    }
}
