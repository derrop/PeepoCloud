...
public abstract class Command {
    ...
    public String getUsage() {
        String usage = PeepoCloudNodeAPI.getInstance().getLanguagesManager().getMessage("command.usage." + this.name);
        if (usage != null)
            return usage;
        return PeepoCloudNodeAPI.getInstance().getLanguagesManager().getMessage("command.usage.notDefined");
    }
