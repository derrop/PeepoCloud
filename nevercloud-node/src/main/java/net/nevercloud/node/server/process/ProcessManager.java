...
public class ProcessManager {
    ...
    void handleProcessStop(CloudProcess process) {
        ...
        if (process instanceof BungeeProcess) {
            ...
            System.out.println(NeverCloudNode.getInstance().getLanguagesManager().getMessage("process.bungee.stopped").replace("%name%", process.toString()));
        } else if (process instanceof ServerProcess) {
            this.serverMemoryAdd.accept(-process.getMemory());
            System.out.println(NeverCloudNode.getInstance().getLanguagesManager().getMessage("process.server.stopped").replace("%name%", process.toString()));
        }
        NeverCloudNode.getInstance().getScreenManager().getProcessScreenManager().disableScreen(process);
    }
    ...
    void handleProcessStart(CloudProcess process) {
        ...
        if (process instanceof BungeeProcess) {
            this.bungeeMemoryAdd.accept(process.getMemory());
            System.out.println(NeverCloudNode.getInstance().getLanguagesManager().getMessage("process.bungee.started").replace("%name%", process.toString()));
        } else if (process instanceof ServerProcess) {
            this.serverMemoryAdd.accept(process.getMemory());
            System.out.println(NeverCloudNode.getInstance().getLanguagesManager().getMessage("process.server.started").replace("%name%", process.toString()));
        }
    }
